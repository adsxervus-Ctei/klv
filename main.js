(function () {
  ...
  /* Nav solidify + menú móvil */
  function initNav() {
    var nav = document.getElementById("nav"), t = document.getElementById("navToggle"), l = document.getElementById("navLinks");
  ...
  function initLightbox() {
    ...
    var idx = 0;
    var srcs = items.map(function (it) { return it.getAttribute("data-full") || it.querySelector("img").src; });
    var open = function (i) { idx = i; img.src = srcs[idx]; lb.classList.add("open"); lb.setAttribute("aria-hidden", "false"); };
    var close = function () { lb.classList.remove("open"); lb.setAttribute("aria-hidden", "true"); };
    var go = function (d) { idx = (idx + d + srcs.length) % srcs.length; img.src = srcs[idx]; };
